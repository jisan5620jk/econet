...
const FeatureCard = ({
  ...
  return (
    <div className="group rounded-[20px] px-6 sm:px-11 pt-6 sm:pt-[52px] pb-6 sm:pb-[46px] relative z-10 overflow-hidden bg-BodyBgDark2-0">
      <div>
        {[
      ...
      <h5 className="font-Outfit font-medium text-[22px] sm:text-2xl lg:text-[22px] 2xl:text-[28px] 2xl:leading-[36px] pt-[24px] pr-10 sm:pr-0 text-white transition-all duration-500 group-hover:text-white max-w-[260px] w-full">
        {featureTitle}
      </h5>
      <div className="absolute top-0 right-0">
        <Link
          to={featureUrl}
          className="size-[50px] flex justify-center items-center font-Outfit text-white text-[28px] bg-BodyBgDark2-0 rounded-full transition-all duration-500 relative z-10 overflow-hidden -rotate-45 before:absolute before:left-0 before:top-0 before:size-full before:bg-PrimaryColor-0 before:transition-all before:duration-500 before:-z-10 before:rotate-180 before:scale-0 group-hover:before:scale-100 group-hover:before:rotate-0 group-hover:text-white"
        >
          {featureBtnIcon}
        </Link>
      </div>
